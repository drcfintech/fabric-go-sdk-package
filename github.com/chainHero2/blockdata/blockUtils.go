...
import (
	...
	"github.com/hyperledger/fabric/protos/common"
	futils "github.com/hyperledger/fabric/protoutil"
	pb "github.com/hyperledger/fabric/fabric-protos-go/peer"
...
func EnvelopeToTrasactionQuery(env *common.Envelope) (*ChainTransactionQuery, error) {
	...
	ta.ChainTransactionConfig.Timestamp = channelHeader.Timestamp.Seconds
	ta.ChainTransactionConfig.ChannelId = channelHeader.ChannelId
	ta.ChainTransactionConfig.TxID = channelHeader.TxId
	if len(spec.GetInput().GetArgs()) != 0 {
		ta.ChainTransactionConfig.TxArgs = spec.GetInput().GetArgs()[1:]
		ta.ChainTransactionConfig.Method = string(spec.GetInput().GetArgs()[0])
	}
	ta.ChainTransactionConfig.Chaincode = spec.GetChaincodeId().GetName()
	ta.ChainTransactionConfig.CreatedFlag = channelHeader.GetType() == int32(common.HeaderType_ENDORSER_TRANSACTION)
	return ta, nil
}
